defmodule ReqAI.Plugins.TokenUsage do
  @moduledoc """
  ...
  * Calculates costs using ReqAI.Model cost metadata
  * Stores usage data in `response.private[:req_ai][:usage]`
  ...
  * Metadata: `%{model: %ReqAI.Model{}}`
  ...
  - `model` - Optional ReqAI.Model struct for cost calculation
  ...
  """
  @spec attach(Req.Request.t(), ReqAI.Model.t() | nil) :: Req.Request.t()
  def attach(%Req.Request{} = req, model \\ nil) do
    req
    |> Req.Request.append_response_steps(token_usage: &__MODULE__.handle/1)
    |> then(fn r ->
      if model, do: Req.Request.put_private(r, :req_ai_model, model), else: r
    end)
  end
  ...
  def handle({req, resp}) do
    with {:ok, usage} <- extract_usage(resp.body),
      ...
      :telemetry.execute([:req_ai, :token_usage], meta, %{model: model})

      # Store usage data in response private for access by callers
      req_ai_data = Map.get(resp.private, :req_ai, %{})
      updated_req_ai_data = Map.put(req_ai_data, :usage, meta)
      updated_resp = %{resp | private: Map.put(resp.private, :req_ai, updated_req_ai_data)}
      {req, updated_resp}
    else
      _ -> {req, resp}
    end
  ...
  @spec fetch_model(Req.Request.t()) :: {:ok, ReqAI.Model.t()} | :error
  defp fetch_model(%Req.Request{private: private, options: options}) do
    case private[:req_ai_model] || options[:model] do
      %ReqAI.Model{} = model -> {:ok, model}
      _ -> :error
    end
  end

  # Calculates cost based on token usage and model pricing
  @spec compute_cost(%{input: non_neg_integer, output: non_neg_integer}, ReqAI.Model.t()) ::
          {:ok, float() | nil} | :error
  defp compute_cost(%{input: _input_tokens, output: _output_tokens}, %ReqAI.Model{cost: nil}) do
    {:ok, nil}
  end

  defp compute_cost(%{input: input_tokens, output: output_tokens}, %ReqAI.Model{cost: cost_map})
