defmodule ReqAI.Capabilities.Reasoning do
  @moduledoc """
  ...
  def verify(model, opts) do
    ...
    case ReqAI.generate_text(model_spec, reasoning_prompt, req_ai_opts) do
      {:ok, %Req.Response{body: content, private: private}} when is_binary(content) ->
        # Handle ReqAI processed response (text body with usage info in private)
        content_trimmed = String.trim(content)
        
        # Check for reasoning tokens in the preserved usage information
        reasoning_tokens = get_in(private, [:req_ai, :usage, :tokens, :reasoning]) || 0
        ...
        if trimmed != "" do
          {:ok,
           %{
             ...
             reasoning_preview: nil,
             has_reasoning_tokens: false,
             warning: "Model returned plain text instead of structured response with reasoning tokens"
           }}
        else
  ...
  defp extract_steps_section(content) do
    cond do
      ...
      match = Regex.run(~r/(?:step|Step)\s*[:\-]?\s*\n?(.{50,200})/s, content, capture: :all_but_first) ->
        List.first(match)
  ...
  defp extract_explanation_section(content) do
    cond do
      ...
      match = Regex.run(~r/(?:reasoning|explanation|thinking|approach|solution)[:\-]?\s*\n?(.{50,200})/is, content, capture: :all_but_first) ->
        List.first(match)
        
      # Match "Here's how" or "Here's why" patterns
      match = Regex.run(~r/(?:here'?s (?:how|why|the))[:\-]?\s*(.{50,200})/is, content, capture: :all_but_first) ->
        List.first(match)
        
      # Match "First," or "To start," patterns
      match = Regex.run(~r/(?:first|to start|initially)[,:\-]\s*(.{50,200})/is, content, capture: :all_but_first) ->
        List.first(match)
