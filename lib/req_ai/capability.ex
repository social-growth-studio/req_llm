defmodule ReqAI.Capability do
  ...
  ## Examples
      ...
      defmodule ReqAI.Capabilities.GenerateText do
        ...
        def verify(metadata, opts) do
          with {:ok, response} <- ReqAI.generate_text(metadata.id, "Hello!", opts),
  ...
  ## Parameters
    ...
    * `model` - The ReqAI.Model struct with metadata

  """
  @callback advertised?(ReqAI.Model.t()) :: boolean()

  @doc """
  Verifies that the capability works correctly with the given model.

  Should make actual API calls using ReqAI's public interface and return
  ...
  """
  @callback verify(ReqAI.Model.t(), keyword()) :: {:ok, term()} | {:error, term()}
end
