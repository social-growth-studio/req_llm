defmodule ReqLLM.Provider.Adapter do
  ...
  HTTP transport is handled separately by `ReqLLM.HTTP.send/2`.
  ...
  ## Example Implementation
      ...
      defmodule MyProvider do
        ...
        def provider_info do
          ReqLLM.Provider.new(:my_provider, "My Provider", "https://api.example.com")
        end
  ...
  A `ReqLLM.Provider.Spec` struct containing:
  ...
  """
  @callback spec() :: ReqLLM.Provider.Spec.t()
  ...
  A `ReqLLM.Provider` struct containing basic provider metadata.

  ## Example

      def provider_info do
        ReqLLM.Provider.new(:openai, "OpenAI", "https://api.openai.com")
      end

  """
  @callback provider_info() :: ReqLLM.Provider.t()
  ...
  ## Example
      ...
      def generate_text(model, messages, opts) do
        with {:ok, request} <- build_request(messages, [], opts),
             {:ok, response} <- ReqLLM.HTTP.send(request, opts),
  ...
  """
  @callback generate_text(ReqLLM.Model.t(), String.t() | [ReqLLM.Message.t()], keyword()) ::
              {:ok, String.t()} | {:error, ReqLLM.Error.t()}
  ...
  ## Example
      ...
      def stream_text(model, messages, opts) do
        ...
        with {:ok, request} <- build_request(messages, [], stream_opts),
             {:ok, response} <- ReqLLM.HTTP.send(request, stream_opts) do
          {:ok, response}
        end
      end

  """
  @callback stream_text(ReqLLM.Model.t(), String.t() | [ReqLLM.Message.t()], keyword()) ::
              {:ok, Enumerable.t()} | {:error, ReqLLM.Error.t()}
