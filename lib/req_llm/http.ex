defmodule ReqLLM.HTTP do
  @moduledoc """
  HTTP transport layer for ReqLLM requests.
  ...
  def send(request, opts \\ []) do
    ...
    model = Keyword.get(opts, :model) || Req.Request.get_private(request, :req_llm_model)

    # Build request with plugins
    request_with_plugins =
      case Req.Request.get_private(request, :req_llm_provider_spec) do
        nil ->
  ...
  def with_provider_spec(request, provider_spec) do
    Req.Request.put_private(request, :req_llm_provider_spec, provider_spec)
  end
