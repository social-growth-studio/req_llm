defmodule ReqLLM.Providers.XAI do
  ...
  Supports xAI's Grok API with features including:
  ...
  Set your xAI API key via environment variable:
  ...
  ## Examples
      ...
      model = ReqLLM.Model.from("xai:grok-2")
  ...
  def attach(request, %ReqLLM.Model{} = model, _opts \\ []) do
    # Get environment variable name from metadata, fallback to XAI_API_KEY
    ...
    unless api_key && api_key != "" do
      raise ArgumentError,
            "xAI API key required. Set via Kagi.put(#{inspect(kagi_key)}, key)"
    end
  ...
  defp get_env_var_name do
    case ReqLLM.Provider.Registry.get_provider_metadata(:xai) do
