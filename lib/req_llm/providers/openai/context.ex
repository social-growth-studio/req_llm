defmodule ReqLLM.Providers.OpenAI.Context do
  @moduledoc false
  defstruct [:context]
  @type t :: %__MODULE__{context: ReqLLM.Context.t()}
end

# Protocol implementation for OpenAI-specific context encoding
defimpl ReqLLM.Context.Codec, for: ReqLLM.Providers.OpenAI.Context do
  def encode_request(%{context: %ReqLLM.Context{messages: messages}}) do
    %{messages: Enum.map(messages, &encode_message/1)}
  end

  # Handle OpenAI response format with "choices" array
