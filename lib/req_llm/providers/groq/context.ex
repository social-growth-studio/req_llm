defmodule ReqLLM.Providers.Groq.Context do
  @moduledoc false
  defstruct [:context]
  @type t :: %__MODULE__{context: ReqLLM.Context.t()}
end

# Protocol implementation for Groq-specific context encoding
# Groq uses OpenAI-compatible format
defimpl ReqLLM.Context.Codec, for: ReqLLM.Providers.Groq.Context do
  def encode_request(%{context: %ReqLLM.Context{messages: messages}}) do
    %{messages: Enum.map(messages, &encode_message/1)}
  end

  # Handle Groq response format with "choices" array (OpenAI-compatible)
  ...
  defp encode_message(%ReqLLM.Message{role: role, content: parts}) do
    %{
      "role" => role_to_groq_role(role),
      "content" => encode_content_parts(parts)
    }
  end

  defp role_to_groq_role(:user), do: "user"
  defp role_to_groq_role(:assistant), do: "assistant"
  defp role_to_groq_role(:system), do: "system"
  defp role_to_groq_role(:tool), do: "tool"
  defp role_to_groq_role(role), do: to_string(role)
