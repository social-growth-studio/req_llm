defmodule ReqLLM.Providers.OpenRouter.Response do
  @moduledoc false
  defstruct [:payload]
  @type t :: %__MODULE__{payload: term()}
end

# Protocol implementation for OpenRouter-specific response decoding
# OpenRouter uses OpenAI-compatible response format with some extensions
defimpl ReqLLM.Response.Codec, for: ReqLLM.Providers.OpenRouter.Response do
  alias ReqLLM.{Response, Context, Message, StreamChunk, Model}

  @doc """
  Decode wrapped OpenRouter response struct.
  """
  ...
  @doc """
  Decode wrapped OpenRouter response struct with model information.
  """
  def decode_response(
  ...
  # SSE Event decoding for streaming responses (OpenRouter uses OpenAI-compatible format)
  defp decode_sse_event(%{event: "completion", data: %{"choices" => [%{"delta" => delta} | _]}}) do
    decode_openrouter_delta(delta)
  end
  ...
  defp decode_openrouter_delta(%{"content" => content})
       when is_binary(content) and content != "" do
    [StreamChunk.text(content)]
  end

  defp decode_openrouter_delta(%{"tool_calls" => tool_calls}) when is_list(tool_calls) do
    tool_calls
    |> Enum.map(&decode_tool_call_delta/1)
    |> Enum.reject(&is_nil/1)
  end

  defp decode_openrouter_delta(_), do: []
...
defmodule ReqLLM.Providers.OpenRouter.ResponseDecoder do
  @moduledoc false
  alias ReqLLM.{Response, Context, Message, StreamChunk, Model}

  def decode_openrouter_json(data, model) when is_map(data) do
    ...
    # Convert OpenRouter choice to StreamChunks
    content_chunks =
      case first_choice do
        %{"message" => message} -> decode_openrouter_message(message)
        %{"delta" => delta} -> decode_openrouter_delta(delta)
        _ -> []
      end
    ...
    provider_meta =
      ...
      |> Map.put("native_finish_reason", Map.get(first_choice, "native_finish_reason"))
  ...
  defp decode_openrouter_message(%{"content" => content})
       when is_binary(content) and content != "" do
    [ReqLLM.StreamChunk.text(content)]
  end

  defp decode_openrouter_message(%{"content" => content}) when is_list(content) do
    content
  ...
  defp decode_openrouter_message(%{"tool_calls" => tool_calls}) when is_list(tool_calls) do
    tool_calls
    |> Enum.map(&decode_tool_call/1)
    |> Enum.reject(&is_nil/1)
  end

  defp decode_openrouter_message(_), do: []
  ...
  # OpenRouter uses OpenAI-compatible usage format but may have extensions
  def parse_usage(%{
        "prompt_tokens" => input,
  ...
  # OpenRouter normalizes finish reasons to standard values
  ...
  def parse_finish_reason("error"), do: :error
