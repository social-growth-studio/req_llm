defmodule ReqLLM.Coverage.Anthropic.StopSequencesTest do
  ...
  describe "stop sequences" do
    test "single stop sequence" do
      ...
      {:ok, response} = LiveFixture.use_fixture("stop_sequences/single_stop", fn ->
    ...
    test "multiple stop sequences" do
      ...
      {:ok, response} = LiveFixture.use_fixture("stop_sequences/multiple_stops", fn ->
        ReqLLM.generate_text(model,
          context: context,
    ...
    test "maximum 4 stop sequences" do
      ...
      {:ok, response} = LiveFixture.use_fixture("stop_sequences/max_four_stops", fn ->
        ReqLLM.generate_text(model,
          context: context,
          stop_sequences: ["Python", "Java", "JavaScript", "Ruby"],  # Exactly 4
          max_tokens: 100
        )
      end)
    ...
    test "stop sequence not reached uses length limit" do
      ...
      {:ok, response} = LiveFixture.use_fixture("stop_sequences/not_reached", fn ->
        ReqLLM.generate_text(model,
          context: context,
    ...
    test "empty stop sequences array is ignored" do
      ...
      {:ok, response} = LiveFixture.use_fixture("stop_sequences/empty_array", fn ->
        ReqLLM.generate_text(model,
          context: context,
          stop_sequences: [],
          max_tokens: 20
    ...
    test "special characters in stop sequences" do
      ...
      {:ok, response} = LiveFixture.use_fixture("stop_sequences/special_chars", fn ->
        ReqLLM.generate_text(model,
          context: context,
          stop_sequences: ["}"],
          max_tokens: 100
  ...
  describe "stop sequences with streaming" do
    test "stop sequence detection in streaming mode" do
      ...
      {:ok, response} = LiveFixture.use_fixture("stop_sequences/streaming_stop", fn ->
