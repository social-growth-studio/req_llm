  ...
  describe "response format parameter" do
    test "response_format parameter should be ignored by Anthropic" do
      ...
      context = ReqLLM.Context.new([
        ReqLLM.Message.user("Respond with valid JSON: {'greeting': 'hello', 'status': 'success'}")
      ])

      # This should work even though response_format is provided - it should be ignored
      {:ok, response} = LiveFixture.use_fixture("response_format/ignored_parameter", fn ->
        ReqLLM.generate_text(model,
          context: context,
          response_format: %{type: "json_object"},  # Should be ignored
          max_tokens: 100
        )
      end)
    ...
    test "json mode enforcement through prompt engineering" do
      ...
      context = ReqLLM.Context.new([
        ReqLLM.Message.system("You must respond only with valid JSON. No other text."),
        ReqLLM.Message.user("Create a JSON object with name='Alice' and age=30")
      ])

      {:ok, response} = LiveFixture.use_fixture("response_format/prompt_enforced_json", fn ->
    ...
    test "structured output via tool calling instead of response_format" do
      ...
      context = ReqLLM.Context.new([
        ReqLLM.Message.user("I need info about Alice, who is 30 years old and her email is alice@example.com")
      ])

      {:ok, response} = LiveFixture.use_fixture("response_format/tool_based_json", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [json_tool],
    ...
    test "text mode is default behavior" do
      ...
      context = ReqLLM.Context.new([
        ReqLLM.Message.user("Tell me about Elixir programming language")
      ])

      {:ok, response} = LiveFixture.use_fixture("response_format/default_text", fn ->
        ReqLLM.generate_text(model, context: context, max_tokens: 100)
      end)
    ...
    test "provider should not send unsupported parameters" do
      ...
      context = ReqLLM.Context.new([
        ReqLLM.Message.user("Simple test message")
      ])

      # Multiple unsupported OpenAI-style parameters that should be filtered out
      {:ok, response} = LiveFixture.use_fixture("response_format/filtered_parameters", fn ->
        ReqLLM.generate_text(model,
          context: context,
          response_format: %{type: "json_object"},  # Should be filtered
          n: 1,                                      # Should be filtered
          logprobs: true,                           # Should be filtered
          presence_penalty: 0.1,                   # Should be filtered
          frequency_penalty: 0.1,                  # Should be filtered
          max_tokens: 50
        )
      end)
