defmodule ReqLLM.Coverage.Anthropic.ToolCallingTest do
  ...
  describe "tool choice parameter" do
    test "tool_choice: auto (default behavior)" do
      ...
      {:ok, response} = LiveFixture.use_fixture("tool_calling/choice_auto", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [weather_tool],
          tool_choice: %{type: "auto"},  # Explicit auto
          max_tokens: 200
        )
      end)
    ...
    test "tool_choice: none (force no tool use)" do
      ...
      {:ok, response} = LiveFixture.use_fixture("tool_calling/choice_none", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [weather_tool],
    ...
    test "tool_choice: any (force tool use)" do
      ...
      context = ReqLLM.Context.new([
        ReqLLM.Message.user("Hello, how are you?")  # Question that doesn't need tools
      ])

      {:ok, response} = LiveFixture.use_fixture("tool_calling/choice_any", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [calculator_tool, weather_tool],
          tool_choice: %{type: "any"},  # Force using one of the tools
          max_tokens: 200
        )
      end)
    ...
    test "tool_choice: specific tool by name" do
      ...
      {:ok, response} = LiveFixture.use_fixture("tool_calling/choice_specific", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [calculator_tool, weather_tool],
    ...
    test "disable_parallel_tool_use option" do
      ...
      {:ok, tool1} = ReqLLM.Tool.new(
        name: "get_weather",
        description: "Get weather",
        parameter_schema: [location: [type: :string, required: true]]
      )

      {:ok, tool2} = ReqLLM.Tool.new(
        name: "get_time",
      ...
      {:ok, response} = LiveFixture.use_fixture("tool_calling/no_parallel", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [tool1, tool2],
  ...
  describe "tool result handling" do
    test "tool call followed by tool result in conversation" do
      ...
      {:ok, first_response} = LiveFixture.use_fixture("tool_calling/initial_call", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [weather_tool],
          max_tokens: 200
      ...
      tool_result = ReqLLM.Message.ContentPart.tool_result(
        tool_call_id, 
        "London: 18°C, partly cloudy, light rain expected"
      )
      
      extended_context = ReqLLM.Context.new([
        ReqLLM.Message.user("What's the weather in London?"),
        ReqLLM.Message.assistant([
          ReqLLM.Message.ContentPart.tool_call(tool_call.name, tool_call.arguments, tool_call_id)
        ]),
        ReqLLM.Message.user([tool_result])
      ])

      {:ok, final_response} = LiveFixture.use_fixture("tool_calling/with_result", fn ->
        ReqLLM.generate_text(model,
      ...
      text_content = final_response.chunks 
                    |> Enum.filter(&(&1.type == :text))
                    |> Enum.map(& &1.text)
                    |> Enum.join()
    ...
    test "multiple tool calls with results" do
      ...
      context = ReqLLM.Context.new([
        ...
        ReqLLM.Message.assistant([
          ReqLLM.Message.ContentPart.tool_call("get_weather", %{"location" => "Paris"}, "call_1"),
      ...
      {:ok, response} = LiveFixture.use_fixture("tool_calling/multiple_results", fn ->
        ReqLLM.generate_text(model,
          context: context,
    ...
    test "tool call with error result" do
      ...
      context = ReqLLM.Context.new([
        ...
        ReqLLM.Message.user([
          ReqLLM.Message.ContentPart.tool_result("call_1", "Error: API endpoint not found (404)")
        ])
      ])

      {:ok, response} = LiveFixture.use_fixture("tool_calling/error_result", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [api_tool],
          max_tokens: 100
  ...
  describe "tool call IDs and correlation" do
    test "tool call IDs are unique and properly formatted" do
      ...
      {:ok, response} = LiveFixture.use_fixture("tool_calling/unique_ids", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [tool1, tool2],
          max_tokens: 300
      ...
      assert length(ids) == length(Enum.uniq(ids))  # All unique
    ...
    test "tool result references correct tool call ID" do
      ...
      {:ok, first_response} = LiveFixture.use_fixture("tool_calling/get_call_id", fn ->
        ReqLLM.generate_text(model,
          context: context,
          tools: [math_tool],
          max_tokens: 200
      ...
      extended_context = ReqLLM.Context.new([
        ReqLLM.Message.user("Calculate 25 * 4"),
        ReqLLM.Message.assistant([
          ReqLLM.Message.ContentPart.tool_call("calculate", %{"expression" => "25 * 4"}, original_id)
        ]),
        ReqLLM.Message.user([
      ...
      {:ok, final_response} = LiveFixture.use_fixture("tool_calling/matched_id_result", fn ->
        ReqLLM.generate_text(model,
          context: extended_context,
          tools: [math_tool],
          max_tokens: 100
