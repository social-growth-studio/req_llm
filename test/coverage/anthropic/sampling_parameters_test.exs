defmodule ReqLLM.Coverage.Anthropic.SamplingParametersTest do
  ...
  describe "temperature parameter" do
    test "low temperature (0.1) for deterministic output" do
      ...
      {:ok, response} = LiveFixture.use_fixture("sampling/low_temperature", fn ->
    ...
    test "high temperature (1.0) for creative output" do
      ...
      {:ok, response} = LiveFixture.use_fixture("sampling/high_temperature", fn ->
  ...
  describe "top_p parameter" do
    test "nucleus sampling with top_p=0.9" do
      ...
      {:ok, response} = LiveFixture.use_fixture("sampling/top_p_sampling", fn ->
      ...
      assert word_count <= 6  # Some flexibility for model interpretation
    end
  end
  ...
  describe "top_k parameter" do
    test "truncated sampling with top_k=50" do
      ...
      {:ok, response} = LiveFixture.use_fixture("sampling/top_k_sampling", fn ->
  ...
  describe "combined sampling parameters" do
    test "temperature, top_p, and top_k together" do
      ...
      {:ok, response} = LiveFixture.use_fixture("sampling/combined_sampling", fn ->
  ...
  describe "parameter validation" do
    test "invalid temperature range should be handled gracefully" do
      ...
      {:ok, response} = LiveFixture.use_fixture("sampling/boundary_temperature", fn ->
    ...
    test "zero values for sampling parameters" do
      ...
      {:ok, response} = LiveFixture.use_fixture("sampling/zero_sampling", fn ->
        ReqLLM.generate_text(model, 
          context: context,
          temperature: 0.0,
          top_k: 1,
