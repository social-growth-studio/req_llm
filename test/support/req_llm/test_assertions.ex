defmodule ReqLLM.Test.Assertions do
  ...
  def assert_reasoning_result(%{} = data) do
    ...
    assert String.length(data.content_preview) <= 100,
           "content_preview should be truncated to 100 chars"

    # Reasoning-specific validations
    if data.has_reasoning_tokens do
      assert data.reasoning_length > 0,
             "reasoning_length should be > 0 when has_reasoning_tokens is true"

      assert is_binary(data.reasoning_preview),
             "reasoning_preview should be present when has_reasoning_tokens is true"
    else
      assert data.reasoning_length == 0,
             "reasoning_length should be 0 when has_reasoning_tokens is false"
    end
  ...
  def assert_stream_result(%{} = data) do
    ...
    assert String.length(data.response_preview) <= 50,
           "response_preview should be truncated to 50 chars"
  ...
  def assert_capability_result(
        %ReqLLM.Capability.Result{status: :passed} = result,
        :passed,
        capability
      ) do
    assert result.capability == capability,
           "Expected capability #{capability}, got #{result.capability}"
  ...
  def assert_capability_result(
        %ReqLLM.Capability.Result{status: :failed} = result,
        :failed,
        capability
      ) do
    assert result.capability == capability,
