defmodule ReqAI.MessagesTest do
  ...
  describe "builder functions" do
    test "creates user, assistant, and system messages" do
      ...
      assert %Message{role: :user, content: "Hello world", metadata: %{priority: "high"}} = user_msg
  ...
  test "validation error cases" do
    ...
    assert {:error, "Message at index 1: Not a valid Message struct"} = validate_messages(invalid_messages)
    
    invalid_message = %Message{role: :invalid_role, content: "Hello"}
    assert {:error, "Invalid role: :invalid_role. Must be :user, :assistant, :system, or :tool"} = validate_message(invalid_message)
    
    invalid_content = %Message{role: :user, content: 42}
    assert {:error, "Content must be a string or list of ContentPart structs, got: 42"} = validate_message(invalid_content)
  end
end
